    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (string file in files)
            {
                ...
                Console.WriteLine("Texture is in DXT" + dxt_version + " format");
                ...
                fileoutName = fileName.Replace(".st", ".dat");
                Console.WriteLine("Output extra data to " + fileoutName);
                ...
                using (Stream fileoutStream = File.OpenWrite(fileoutName))
                {
                    fileoutStream.Write(Encoding.ASCII.GetBytes(textureName), 0, Encoding.ASCII.GetBytes(textureName).Length);
                    fileoutStream.Write(new byte[32 - Encoding.ASCII.GetBytes(textureName).Length], 0, 32 - Encoding.ASCII.GetBytes(textureName).Length);
                    fileoutStream.Write(extraBytes, 0, extraBytes.Length);
                    fileoutStream.Close();
                }
            }
        }
